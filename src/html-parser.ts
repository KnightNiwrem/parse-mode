import type { EntityTag } from "./format.ts";

import { a, b, blockquote, code, i, pre, s, spoiler, u } from "./format.ts";

/**
 * `custom_emoji` entity tag. Incompatible with `code` and `pre`.
 * @param customEmojiId The custom emoji ID.
 */
function emoji(customEmojiId: string): EntityTag {
  return { type: "custom_emoji", custom_emoji_id: customEmojiId } as EntityTag;
}
          ...
          switch (tagName) {
            ...
            case "span":
              if (
                ...
                // class attribute must be tg-spoiler for <span> tags
            ...
            case "tg-emoji":
              if (!attributes["emoji-id"]) {
                // emoji-id attribute is mandatory for <tg-emoji> tags
                textBuffer += `<${tagContentBuffer}>`;
              } else {
